            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">تعديل صنف</h2>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form class="row" action="{{route('product.update',$product->id)}}" method="POST">
                                        @csrf
                                        <div class="col-xl-4 col-md-6 col-sm-12 mb-2">
                                            <label for="credit-card">اسم صنف</label>
                                            <input type="text" name="name" value="{{$product->name}}" class="form-control credit-card-mask" placeholder="اسم الصنف"  />
                                                ...
                                                @if ($stores && $stores->count() > 0)
                                                    @foreach ($stores as $store)
                                                        <option value="{{ $store->id }}"
                                                            {{ $product->store_id == $store->id ? 'selected' : null }}>
                                                            {{ $store->name }}</option>
                                        ...
                                        <div class="col-xl-4 col-md-6 col-sm-12 mb-2">
                                            ...
                                            <input type="text" name="count" value="{{$product->count }}" class="form-control credit-card-mask" placeholder=" الكمية"  />
                                            @error('count')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-xl-4 col-md-6 col-sm-12 mb-2">
                                            <label for="credit-card">السعر</label>
                                            <input type="text" name="price" value="{{ $product->price }}" class="form-control credit-card-mask" placeholder=" السعر"  />
